...
- **Splits** (`/splits`, linked from the "You're Owed"/"You Owe" stat) — manual Splitwise-like IOU ledger (Splitwise's real API is Pro-only now, so this is self-hosted instead): add entries per person (amount, direction, what for), grouped by person with a running net. Settling a single entry offers to link the real bank transaction that paid it back (see Transfers below) instead of just flipping a flag — closes the loop between Splits and Activity instead of bookkeeping the same repayment twice. Bulk "Settle up with X" stays a plain flag-flip, no linking.
...
**Transfers** — money passing through you (a split-bill repayment, paying/getting paid back for someone else, a refund) isn't real income or spending. The same sheet has a "This is a transfer" toggle (+ optional note, e.g. a name) — `is_transfer`/`transfer_note` on the transaction. Transfers are excluded from Total Received/Total Spent everywhere (dashboard stats, Summary) but still count toward Net Balance, since the money genuinely moved; Summary shows a separate "Transfers" line so the volume isn't invisible.

**Month attribution** — a transaction's real date drives Net Balance, but Summary/dashboard reporting can be overridden per-transaction (`attributed_month`, format `YYYY-MM`) via a `‹ Month ›` stepper in the same sheet — for salary etc. landing a few days into the wrong calendar month. Tagging something "Salary" within the last few days of a month auto-prompts whether it should count toward next month instead.
